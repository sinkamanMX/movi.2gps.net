...
function fun_load_datatable(){
    ...
    funTable = $('#fun_table').dataTable({
      ...
      "bAutoWidth": false,
      "bSortClasses": false,
      "sAjaxSource": "index.php?m=mUnidadMedida&c=mUnidadMedida",
      "aoColumns": [
	    { "mData": " ", sDefaultContent: "" },
		{ "mData": "DESCRIPCION_UNIDAD", sDefaultContent: "","bSearchable": false,"bVisible":    false },
        { "mData": "DESCRIPCION_UNIDAD", sDefaultContent: "" },
        { "mData": "", sDefaultContent: ""}
      ...
      "aoColumnDefs": [
        {"aTargets": [0],
          ...
          "mRender": function (data, type, full) {
            ...
            edit = "<td><div onclick='fun_abrir_formulario("+full.ID_UNIDAD_MEDIDA+",2);' class='custom-icon-edit-custom'>"+
            ...
            del = "<td><div onclick='fun_delete_function("+full.ID_UNIDAD_MEDIDA+");' class='custom-icon-delete-custom'>"+
                    "<img class='total_width total_height' src='data:image/gif;base64,R0lGODlhAQABAJH/AP///wAAAMDAwAAAACH5BAEAAAIALAAAAAABAAEAQAICVAEAOw=='/>"+
                    "</div></td>";
...
function fun_abrir_formulario(id,op){
	//alert(id+"/"+op)
	if(op==1){$("#fun_dialog_formu").dialog('option', 'title', 'Agregar Unidad de Medida');}
	if(op==2){$("#fun_dialog_formu").dialog('option', 'title', 'Editar Unidad de Medida');}
	      $.ajax({
          url: "index.php?m=mUnidadMedida&c=mFormulario",
          type: "GET",
          data: {
			  id : id,
			  op : op
...
function fun_val_data(){
	...
	if($("#fun_nom").val().length == 0){
		...
		$('#dialog_message').html('<p align="center"><span class="ui-icon ui-icon-alert" style="float:left; margin:0 1px 25px 0;">'+
		                          '</span>Debe escribir el nombre de la unidad de medida.</p>');
...
/*	var pars="";
	...
	$.ajax({
		url: "index.php?m=mUnidadMedida&c=mSaveFuncion",
        type: "GET",
		data:{
			nom : $("#fun_nom").val(),
			op  : $("#fun_hop").val(),
...
function fun_delete_function(q){
    ...
    $('#fun_dialog_confirm').html('<p><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;">'+
								  '</span>&iquest;Desea borrar esta unidad de medida?</p>');
...
function fun_send_delete(q){
	...
	$("body").css("cursor", "progress");
      $.ajax({
          url: "index.php?m=mUnidadMedida&c=mDelFuncion",
          type: "GET",
          data: { 
		  fun_id : q
		  },
