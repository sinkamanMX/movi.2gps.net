  ...
  if ($con){
    ...
    if (marca_tablas($instancia)){
		echo "Paso 2 Registros Marcados  ... <br>";
		echo $sql = "SELECT DESCRIPTION,
		         COD_TABLE,
		         COD_ENTITY,
		...
		if ($qry = mysql_query($sql,$con)){
			while ($row = mysql_fetch_object($qry)){
			  echo $last = "ALERTLAST".$row->NOMBRE;
			  analiza_entidad($row->COD_ENTITY,$row->DESCRIPTION,$last);
			  libera_tabla($row->COD_ENTITY);
			}
    	...
    	echo "no se pudieron marcar";
    }
    mysql_close($con);
  }
  
  
	function analiza_entidad($entity,$tabla,$last){
		global $con;
	    $alertas = array();
	    $q = 0;
	    $alertas = busca_unidad_alerta($entity); 
	    
	    $clientes = array();
	    $clientes = busca_clientes($entity); 
	    $num_cli = count($clientes);
	    $num = count($alertas);
	    if ($num > 0){
	      echo "<br> Paso 3 ... Busca Alertas Pendientes <br>";
		  ...
		  if ($qry = mysql_query($sql,$con)){
		    while ($row = mysql_fetch_object($qry)){
		      ...
		      for ($z=0; $z < $num_cli; $z++){
			    ...
			    if ($evtp <> ""){
			      update_last_pdi($evtp,$last,$geo[0][0],$geo[0][1]);    
			    }
			    ...
			    if ($num_geo > 0){
			      for($x=0; $x < $num_geo; $x++){  
			        ...
			        $evt = revisa_geo_last($last,$geo[$x][0],$geo[$x][1],'geo');
			        echo "geo cumple ".$evt."<br>";
			        if ($evt <> ""){
				      update_last_geo($evt,$last,$geo[$x][0],$geo[$x][1]);
				      //aqui estaba el for 
				      for ($y=0; $y < $num; $y++){
	...
	function marca_tablas($instancia){
		global $con;
		$res = false;
		$sql = "UPDATE SAVL1470 
    ...
    function busca_unidad_alerta($entity){
    	global $con;
	    $alertas = array();
	    $columna = dia($con);
		...
		//echo "<br> alertas programadas <br>".$sql."<br>";
		if ($qry = mysql_query($sql,$con)){
		  $i = 0;
		  while ($row = mysql_fetch_object($qry)){
	...
	function busca_clientes($entity){
		global $con;
		$alertas = array();
		$columna = dia($con);
	...
	function update_last($last,$row){
		global $con;
		$sql='';
		echo "Paso 4 ... Validando tabla";
		if (valida_last($last) > 0){
	      $sql = "UPDATE ".$last."
				  ...
				  echo "Paso 4 ... Se debe de actualizar la tabla ";
			...
			$sql = "INSERT INTO ".$last."
              SET GPS_DATETIME 			= '".	((isset($row->GPS_DATETIME)) ? $row->GPS_DATETIME : 'NULL'	)."', 
					 ...
					 longitude 			= ".	$row->longitude			.",
					 latitude 			= ".	$row->latitude." ";   
			echo "Paso 4 ... Se debe de insertar la tabla ";			 
		}
		
		mysql_query($sql,$con);
	}
	
	function valida_last($last){
		global $con;
	...
	function calcula_geopunto($latitude,$longitude,$cliente){
		...
		$conexion = mysqli_connect("188.138.40.249", "sa", "$0lstic3$", "ALG_BD_CORPORATE_MOVI");
		if ($conexion) {
			...
			if($query = mysqli_query($conexion, $sql)){
				...
				echo "Error query: " . mysqli_error(); 	
		 	}
			 
			 mysqli_close($conexion);		    		    
		}else{
			echo "mysqli_connect failed: " . mysqli_connect_error();
		}  		
		return $geo;
	}
	
	function revisa_geo_last($last,$id,$nombre,$tipo){
		global $con;
		$evt = "";
	...
	function update_last_geo($evt,$last,$id,$nombre){
		global $con;
		switch ($evt){
		  case "IN":
	...
	function update_last_pdi($evt,$last,$id,$nombre){
		global $con;
		switch ($evt){
		  case "IN":
	...
	function evalua_expresion($where,$con2,$last){
		global $con;
		$res = false;
	...
	function inserta_aviso($con2,$id,$alerta,$destinos,$unidad,$lat,$lon,$gps_datetime,$pkh,$entity){
		global $con;
	...
	function borra_registro($id,$tabla,$con2){
		global $con;
		/*$con = mysql_connect("188.138.40.249","sa","$0lstic3$");
		if ($con){*/
		    $base2 = mysql_select_db("ALG_BD_CORPORATE_ALERTAS_MOVI",$con);
