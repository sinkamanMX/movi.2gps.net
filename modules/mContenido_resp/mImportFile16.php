...
if ($permiso){
	if (!in_array($extension_archivo, $extensiones_permitidas)){
		...
		$mensaje .= '<tr><td colspan="2" align="center">&nbsp; 1</td></tr>';
			...
			// $mensaje = 'El archivo <b>' . $nombre_archivo . '</b> ha sido subido correctamente';
			 ...
			 //echo "caso = 1";
			   ...
			   $x_ban = valida_excel($archivo_x);
        	    ...
        	    $mensaje =  '<table border="0" width="100%">';
        		$mensaje .= '<tr><td colspan="2" align="center">&nbsp; 2</td></tr>';
        		//$mensaje .= '<tr><td colspan="2" align="center">&nbsp;</td></tr>';
                $mensaje .= '<tr><td colspan="2" align="center" >El archivo: <b>'.$nombre_archivo.'</b></td></tr>';
        		$mensaje .= '<tr><td colspan="2" align="center">No puede ser procesado</td></tr>';
...
if(count($sap_no)>0){
		$mns .=  '<table border="0" width="100%">';
        $mns .= '<tr><td align="center">SAP Cliente(s) Inexistente(s)</td></tr>';	 
		$mns .= '<tr><td>';
...
for ($row=2;$row<=$excel->rowcount();$row++) {
	if($excel->val($row,'G')!=""){
		if (!in_array($excel->val($row,'G'), $und_nm)){
...
if(count($und_no)>0){
		$mns .= '<table border="0" width="100%">';
        $mns .= '<tr><td align="center">Unidad(es) Inexistente(s)</td></tr>';	 
		$mns .= '<tr><td>';
...
for ($row=2;$row<=$excel->rowcount();$row++) {
	if($excel->val($row,'E')!=""){
		if (in_array($excel->val($row,'E'), $und_id)){
...
if(count($ent_ex)>0){
		$mns .= '<table border="0" width="100%">';
        $mns .= '<tr><td align="center">Pedido(s) Inexistente(s)</td></tr>';	 
		$mns .= '<tr><td>';
for($x=0; $x<count($ent_ex); $x++){
	...
	$mns.='</table>';
		...
		echo $mns;
if(count($sap_no)==0 && count($und_no)==0 && count($ent_ex)==0){
	//VALIDAR DESPACHO
	// OBTENER DESPACHOS 
	 $dsp = array();
	 $dsp_in = array();
	...
	//Validar despachos
		...
		for ($row=2;$row<=$excel->rowcount();$row++) {
			...
			if($excel->val($row,'B')!=""){
				if ((!in_array($excel->val($row,'B'), $dsp)) && (!in_array($excel->val($row,'B'), $dsp2))){
					...
					$f=explode(".",$excel->val($row,'C'));
					$h=explode(" ",$excel->val($row,'A'));
					$hi=($h[1]=="a.m.")?$h[0]:(substr($h[0],0,2)+12).(substr($h[0],2,8));
					$dti="'".$f[2]."-".$f[1]."-".$f[0]." ".$hi."'";
					str_replace(" a.m.","",$dti);
					str_replace(" p.m.","",$dti);
		...
		if($vls!=""){
			...
			if($qry_d==true){
				...
				//$mns .= '<tr><td align="center">Pedido(s) Inexistente(s)</td></tr>';	 
				$mns .= '<tr><td>';
				$mns .="Se insertaron ".count($dsp2)." despacho(s) con exito";
				$mns .='</td></tr>';
				$mns .='</table>';
...
$d=0;
		for ($row=2;$row<=$excel->rowcount();$row++) {
			if($excel->val($row,'B')!=""){
					...
					//$h=explode(" ",$excel->val($row,'N'));
					//$he=($h[1]=="a.m.")?$h[0]:(substr($h[0],0,2)+12).(substr($h[0],2,8));
					$dte="'".$f[2]."-".$f[1]."-".$f[0]." ".$excel->val($row,'J').":00'";
					str_replace(" a.m.","",$dte);
					str_replace(" p.m.","",$dte);				
					
				$valus=($valus=="")?"(".$dsp_in[$d].",".$und_ok[$d].",'".date('Y-m-d H:i:s')."',1,0)":$valus.",(".$dsp_in[$d].",".$und_ok[$d].",'".date('Y-m-d H:i:s')."',1,0)";
				$vls_i=($vls_i=="")?"(".$dsp_in[$d].",2,".$sap_in[$d].",".$idu.",".$excel->val($row,'E').",'".$excel->val($row,'M')."',".$dte.",'".date('Y-m-d H:i:s')."')":$vls_i.",(".$dsp_in[$d].",2,".$sap_in[$d].",".$idu.",".$excel->val($row,'E').",'".$excel->val($row,'M')."',".$dte.",'".date('Y-m-d H:i:s')."')";
		...
		$qry_f = $db->sqlQuery($sql_f);
			if($qry_f==true){
		$sql_e = "INSERT INTO DSP_ITINERARIO (ID_DESPACHO, ID_ESTATUS, COD_GEO, COD_USER, ITEM_NUMBER, COMENTARIOS, FECHA_ENTREGA, CREADO) VALUES ".$vls_i;
		$qry_e = $db->sqlQuery($sql_e);
		if($qry_e==true){			
				$mns .= '<table border="0" width="100%">';
				//$mns .= '<tr><td align="center">Pedido(s) Inexistente(s)</td></tr>';	 
				$mns .= '<tr><td>';
				$mns .="Se insertaron ".$excel->rowcount()." registros";
				$mns .='</td></tr>';
				$mns .='</table>';
				echo $mns;		
			}
