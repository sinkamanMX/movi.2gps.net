...
$sl = "SELECT *,CURRENT_TIMESTAMP AS T1 FROM DSP_DESPACHO a
       ...
       AND a.ID_DESPACHO = ".$_GET['unidad']." AND k.ID_USUARIO = ".$_GET['cod_cli'];
  ...
  $sql ="SELECT c.CAPACITY_CARRY_KG,a.ID_DESPACHO,c.COD_ENTITY,a.DESCRIPCION AS DES ,
...
IF(c.CAPACITY_CARRY_KG=0,0,SUM(d.VOLUMEN)/c.CAPACITY_CARRY_KG) AS CAPACIDAD
			FROM DSP_DESPACHO a 
			INNER JOIN DSP_UNIDAD_ASIGNADA b ON a.ID_DESPACHO = b.ID_DESPACHO 
	    ...
	    if($count>0){	
				...
				$tpl->assign_vars(array(
						...
						'CAPACIDAD'          => asignacion_carga($row['VOLUMEN'],$row['CAPACITY_CARRY_KG']),
						'RESTO'              => asignacion_carga($row['VOLUMEN'],$row['CAPACITY_CARRY_KG']) 
			  	));	
...
//----------------------------------
function asignacion_carga ($volumen,$capacidad){
	$carga_restante  = 0;
/*	if($volumen > $capacidad){
	  $carga_restante = $volumen - $capacidad;
    }*/
    if($capacidad != 0){
    	 $carga_restante = round(($volumen * 100)/$capacidad);
    }
