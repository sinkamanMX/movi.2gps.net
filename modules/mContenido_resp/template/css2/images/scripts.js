...
function gdr_edp(){
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////
		var ajax = nuevoAjax();
  		ajax.open("GET", "index.php?m=mContenido2&c=mGdrEdtp&cte="+a+"&idp="+b+"&dte="+c+"&obs="+d+"&cst="+e+"&pld="+f+"&ide="+ide,true);
  		ajax.onreadystatechange=function() {
  	 	if (ajax.readyState==4) {
			$("#dialog_pb" ).dialog('close');	
	     	var result =ajax.responseText;
...
"Google Maps",{ minZoomLevel: 1, maxZoomLevel: 19, numZoomLevels: 20  } // the default
); 
var lonLat = new OpenLayers.LonLat(-101.62354,21.64307);
var zoom;
 			zoom=3;		
  			map.addLayers([mas,layer_ms]);
...
function createUnits() {
	...
	style_blue.fontColor="#000000";
               ...
               pointFeature = new OpenLayers.Feature.Vector(points[i],null,style_blue);
...
$('#dialog-message').html('<p align="center"><span class="ui-icon ui-icon-alert" style="float:left; margin:0 1px 25px 0;"></span>El punto no pudo ser reprogramado</p>');
 ...
 function r_filtro1(){
	...
	fecha=date;
	$('#list_form_vj').html('<div class="demo"><br/><br/><br/><br/>Cargando Datos<br/><div id="progressbar" style=" width:300px;" ></div></div>');
...
function admap(){	
	...
	OpenLayers.Control.CustomNavToolbar = OpenLayers.Class(OpenLayers.Control.Panel, {
		...
		map = new OpenLayers.Map('tabs-04', {
 				controls: [
...
function onload1(){
	try{
		...
		units.splice(0,angles.length);
		infos.splice(0,infos.length);
		iconos.splice(0,iconos.length);
		valid.splice(0,angles.length);
...
document.getElementById('combo').innerHTML="Cargando...";
	...
	var colortrue= "#F32424";
	 ...
	 var path = "index.php?m=mContenido2&c=mGetPoints";
...
'</table>';
		...
		if(flag_p1==0){
			...
			var textselec=" <select name='selectunid' id='selectunit' style='width: 170px;' onchange='endload(this.value)'>"+
