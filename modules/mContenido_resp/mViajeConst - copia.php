...
////--------------------------------------------------------------------------------TIEMPOS 
  $sql1 = "SELECT CONCAT(PG.DESCRIPTION,' - ',P.VOLUMEN,' ',V.DESCRIPCION) AS
DESCRIPTION,
   AU.COD_ENTITY,
   P.COD_GEO,
   P.ID_DESPACHO,
  ...
  IF(P.FECHA_FIN > CURRENT_TIMESTAMP,
TIME_TO_SEC(CAST(CURRENT_TIMESTAMP AS TIME)),
...
FROM
   ...
   LEFT OUTER JOIN SAVL_G_PRIN PG ON (P.COD_GEO = PG.COD_OBJECT_MAP)
...
GROUP BY
 ...
 $query2 = $db->sqlQuery($sql2);
	...
	$upos = $Positions->obtener_ureporte_2($segmento[$i][2],$dayhr,$idc);
	...
	if($upos != 0){
				...
				for($k=$limit1;$k<$lim;$k++){
					...
					if($rest<1800 && $k!=0){
								...
								$direccion  = $Positions->direccion_s($upos[$q][5],$upos[$q][6]);
							if($userAdmin->codif($direccion)==''){
					    $direccion1  = $Positions->direccion($upos[$q][5],$upos[$q][6]);
					...
					$new_dir=$userAdmin->codif($direccion);
						...
						$pdi="Sin PDI cercano";
								...
								$message= "<tr><td><img src='".$upos[$q][9]."' width='20px' heigth='20px'> </td>".
								      "<td>" .$segmento[$i][1]."</td>".
									  "<td>".$userAdmin->codif($upos[$q][3])."</td>".
									  "<td>".$upos[$q][2]."</td>".
									  "<td>".$userAdmin->codif($new_dir)."</td>".
									  "<td>".$pdi."</td></tr>";
