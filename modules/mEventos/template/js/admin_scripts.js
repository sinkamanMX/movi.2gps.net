...
function evt_load_datatable(){
    ...
    oTable = $('#evt_table').dataTable({
      ...
      "bAutoWidth": false,
      "bSortClasses": false,
      "sAjaxSource": "index.php?m=mEventos&c=adminGetTable&id_client="+client,
      "aoColumns": [
      ...
      "aoColumnDefs": [
        {"aTargets": [0],
          ...
          "mRender": function (data, type, full) {
                /*edit = '<td><span class="ui-icon ui-icon-pencil" onclick="evt_edit_function('+full.COD_EVENT+');"></span></td>';            
                del = '<td><span class="ui-icon ui-icon-trash" onclick="evt_delete_function('+full.COD_EVENT+','+'\''+full.DESCRIPTION+'\');"></span></td>';*/
                edit = "<td><div onclick='evt_edit_function("+full.COD_EVENT+");' class='custom-icon-edit-custom'>"+
                        ...
                        "</div></td>";
                del = "<td><div onclick='evt_delete_function("+full.COD_EVENT+");' class='custom-icon-delete-custom'>"+
                        "<img class='total_width total_height' src='data:image/gif;base64,R0lGODlhAQABAJH/AP///wAAAMDAwAAAACH5BAEAAAIALAAAAAABAAEAQAICVAEAOw=='/>"+
                        "</div></td>";                                        
  ...
  function evt_send_delete(value){
     var client = $("#global_id_client").val();      
    $.ajax({
        url: "index.php?m=mEventos&c=adminDelRow",
        type: "GET",
        dataType : 'json',
        data: { data: value ,id_client :client},
        success: function(data) {
  ...
  function evt_validar_nvo(){
   ...
   if($("#evt_txt_icono").val()=="NULL" || $("#evt_txt_icono").val()==""){
      $('#evt_dialog_message').html('<p align="center"><span class="ui-icon ui-icon-alert" style="float:left; margin:0 1px 25px 0;">'+
  ...
  function evt_send_row(){
     var client = $("#global_id_client").val();      
      $.ajax({
          url: "index.php?m=mEventos&c=adminSetRow",
          type: "GET",
          dataType : 'json',
          data: { 
            id_client :client ,
            ...
            prio    :  $("#evt_sel_prio").val(),
            color   :  $("#evt_txt_color").val(),
			      icono   :  $("#evt_txt_icono").val(),
			      visible :  $("#evt_sel_visible").val(),
			      alerta  :  $("#evt_sel_alert").val()
          },
          success: function(data) {            
            var result = data.result; 
