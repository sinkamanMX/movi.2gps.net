...
function user_load_datatable(){
    ...
    oTable = $('#user_table').dataTable({
      ...
      "bAutoWidth": false,
      "bSortClasses": false,
      "sAjaxSource": "index.php?m=mUsuarios&c=adminGetTable&id_client="+client,
      "aoColumns": [
  ...
  function user_edit_function(value){
   ...
   user_units_com = Array();
	  ...
	  $.ajax({
          url: "index.php?m=mUsuarios&c=adminGetRow",
          type: "GET",
          ...
          success: function(data) {            
            ...
            $('input[name=usr_chk_tipo]').change(function(){
                if($('input[name=usr_chk_tipo]').is(':checked')){
                        $("#usr_div_tipo").removeClass("invisible").addClass("visible"); 
                    } else {
                        $("#usr_div_tipo").removeClass("iisible").addClass("invisible");
                    }
            });
          }
      });
    ...
    if(user_units_com.length>-1){
      var lista=user_units_com.toString();
        ...
        $('#com_sortable1').empty();
        $.ajax({
            url: "index.php?m=mUsuarios&c=adminGetRowCom",
            type: "GET",
  ...
  function user_load_permisos(){
      ...
      $.ajax({
          ...
          type: "GET",
          data: { data: $('#user_select_perfil').val(),id_client :client },
          success: function(data) {            
            var result = data; 
            $('#user_accordion').html('');
            if(result!="no-data"){
  ...
  function user_load_units(){
        var client = $("#global_id_client").val();    
      $.ajax({
          url: "index.php?m=mUsuarios&c=adminGetUnits",
          type: "GET",
          data: { data: $("#user_txt_id").val(),
				  id_client :client,
  ...
  function user_send_row(){
    ...
    $('#com_sortable1 li').each(function(id){
	 ...
	 var atencion = ($('input[name=usr_chk_tipo]').is(':checked')==true) ? 1: 0;
      $.ajax({
          url: "index.php?m=mUsuarios&c=adminSetRow",
          type: "GET",
          dataType : 'json',
          data: { 
            id_client: client,
