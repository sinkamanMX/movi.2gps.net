...
**/
	...
	$a_respuestas = array(
		array("id"=>"M",'name'=>'Monitorista' ),
		array("id"=>"S",'name'=>'Supervisor' )
	);
	...
	if(isset($_GET['data']) || $_GET['data'] !="0"){	
		...
		$sql_units = "SELECT ADM_USUARIOS_GRUPOS.COD_ENTITY, CONCAT(PLAQUE,'-', DESCRIPTION) AS UNIDAD,
	...
	$where_module = 'ID_SUBMENU IN (SELECT ID_SUBMENU 
						FROM ADM_SUBMENU_CLIENTES WHERE ID_CLIENTE = '.$id_client.')'.
						'AND TIPO ="M" ';
	...
	$query_e = $db->sqlQuery($sql_e);		
  	...
  	if($count_e>0){
  		while($row_e = $db->sqlFetchArray($query_e)){
			...
			$id_permiso_e   = ($info_permiso_e['ID_PERMISO']) ? $info_permiso_e['ID_PERMISO']: '';  						 			$selected_e     = ($info_permiso_e['ID_PERMISO']) ? 'icon_unit_selected': 'icon_unit_unselected';
 			$disabled_e     = (!$info_permiso_e['ID_PERMISO']) ? 'disabled=""': '';						
			  
  			$excepciones .= '<tr><td>'.
  							'<div id="user_acordeon_icon_'.$row_e['ID_SUBMENU'].'" class="'.$selected_e.
  								'" onclick="user_select_menu('.$row_e['ID_SUBMENU'].')"></div>'.
	...
	$checked_tipo = '';
	$visible_tipo = 'invisible'; 
	$atencion = $dbf->getRow('SRV_USUARIOS','ID_USUARIO = '.$id_row);
	if($atencion){
		$visible_tipo = 'visible';
		$checked_tipo = 'checked';			
	} 
	$tipo 	  = $Functions->cbo_from_array($a_respuestas,@$atencion['TIPO_USUARIO']);
	
	$tpl->set_filenames(array('mSetRow' => 'admintSetRow'));
	$tpl->assign_vars(array(
		...
		'CBO_TIPO'	=> $tipo,
		'CBO_STYLE' => $visible_tipo,
		'CHECK_TIPO'=> $checked_tipo		
	));	
	$tpl->pparse('mSetRow');	
?>