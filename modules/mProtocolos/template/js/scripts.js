...
function proc_load_datatable(){
    oTable = $('#proc_table').dataTable({
      ...
      "bAutoWidth": false,
      "bSortClasses": false,
      "sAjaxSource": "index.php?m=mProtocolos&c=mGetTable",
      "aoColumns": [
  ...
  function proc_edit_function(value){
      $.ajax({
          url: "index.php?m=mProtocolos&c=mGetRow",
          type: "GET",
          data: { data: value  },
          success: function(data) {            
            ...
            $('#global_id_client').change(function() {
              proc_show_unidades($(this).val());
            });              
            $("#proc_tabs" ).tabs(); 
  ...
  function proc_show_unidades(id_cliente){
    if(id_cliente>0){
      $('#proc_cbo_unit').empty();
      $.ajax({
          url: "index.php?m=mProtocolos&c=mGetUnits",
          type: "GET",
          dataType : 'json',
          data: { data: id_cliente },
          success: function(data) {
            var result = data.units; 
            var counter=0;
            $.each(result,function(index,record){
                $("#proc_cbo_unit").append('<option value="'+record.ID+'">'+record.NAME+'</option>');
                counter++;
            });
            if(counter==0){
                $("#proc_cbo_unit").append('<option value="-1">Sin Unidades</option>');
            }
          }
      });      
    }
