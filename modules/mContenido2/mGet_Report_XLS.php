...
**/
	...
	//--------------------------- Modificada BD y Encabezado------------------------
$sql_f="SELECT D.DESCRIPCION AS VJE, U.DESCRIPTION AS UND FROM DSP_DESPACHO D
INNER JOIN DSP_UNIDAD_ASIGNADA A ON A.ID_DESPACHO=D.ID_DESPACHO
INNER JOIN SAVL1120 U ON U.COD_ENTITY=A.COD_ENTITY
WHERE D.ID_DESPACHO=".$_GET['idd'];
	$qry_f = $db->sqlQuery($sql_f);
	$row_f=$db->sqlFetchArray($qry_f);
	...
	//global $db,$funciones,$recorrido;
   	...
   	//$tiempo1 = explode(" ",$fechaInicio);
	//$tiempo2 = explode(" ",$fechaFin);
	//$hr1 = '';
	//$hr2 = '';
	...
	if($_GET['idd']){
                             ...
                             $fechaActual = date("d-m-Y H:i:s");
                    				...
                    				$objPHPExcel->getProperties()->setCreator("ALG")
                    											 ...
                    											 ->setTitle("Office 2007 XLSX")
                    								 			 ...
                    								 			 ->setDescription("Reporte de puntos de entrega.")
							...
							$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', 'Reporte puntos de entrega del viaje '.$row_f['VJE']);
							...
							$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E2', 'Generado: '.$fechaActual);
                            
							$objPHPExcel->getActiveSheet()->mergeCells('A2:B2');
							$objPHPExcel->setActiveSheetIndex(0)
										->setCellValue('A2', 'Viaje: '.$row_f['VJE']);
										
							$objPHPExcel->getActiveSheet()->mergeCells('C2:D2');
							$objPHPExcel->setActiveSheetIndex(0)
										->setCellValue('C2', 'Unidad: '.$row_f['UND']);			
										
							
							$objPHPExcel->setActiveSheetIndex(0)
								->setCellValue('A3', 'Identificador de punto')
								->setCellValue('B3', 'Punto de interes')
								->setCellValue('C3', 'Fecha de entrega')
            					->setCellValue('D3', 'Observaciones')
                                ->setCellValue('E3', 'Estatus')
                                ->setCellValue('F3', 'Cuestionario');
			...
			//$unidadesc = $Positions->get_data_unit($id_unidad);
				...
				/**********************************************************************************************/
					...
					//	$generate = $funciones_cv->obtener_recorrido($id_unidad,$table_hist,$opciones,$dias[$i],$id_itn);	
						//$generate = $funciones_cv->obtener_recorrido($id_unidad,$table_hist,$opciones,$dias[$i],$id_itn,$hr1,$hr2);	
                       /* if($generate){
                            //Visitas del ITN//
                            ...
                            if($a_recorrido){
                               ...
                               $tpl->assign_block_vars('reportes',array(
									...
									'distrecorrida' =>ceil($kms_recorrido),
                               ...
                               if($contador>0){
								...
								$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $row, ceil($kms_recorrido)." Km/h");
                       ...
                       // if($generate){
                            ...
                            if($a_recorrido){
                               ...
                               /*if($contador>0){
								...
								$sql="SELECT E.DESCRIPCION AS STS, P.DESCRIPTION AS PDI, D.ITEM_NUMBER, D.COMENTARIOS, D.FECHA_ENTREGA, C.DESCRIPCION AS CST FROM DSP_ITINERARIO D
								LEFT JOIN DSP_DOCUMENTA_ITINERARIO B ON B.ID_ENTREGA=D.ID_ENTREGA
								LEFT JOIN CRM2_CUESTIONARIOS C ON C.ID_CUESTIONARIO=B.ID_CUESTIONARIO
								INNER JOIN DSP_ESTATUS E ON E.ID_ESTATUS=D.ID_ESTATUS
								INNER JOIN SAVL_G_PRIN P ON P.COD_OBJECT_MAP=D.COD_GEO
								WHERE D.ID_DESPACHO=".$_GET['idd']." ORDER BY D.FECHA_ENTREGA,PDI;";
								$qry = $db->sqlQuery($sql);
								...
								$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row, "".$rw['ITEM_NUMBER']);
								$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $rw['PDI']);
								$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $rw['FECHA_ENTREGA']);
                                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $row,	$rw['COMENTARIOS']);
								$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $row,	$rw['STS']);
                                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $row,	$rw['CST']);
                                //$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, $row,	$rw['ITEM_NUMBER']);
								$row++;
		...
		//}//FOR
			//if($contador>0){
				...
				$filename  = "Repentregas".$row_f['VJE'].'_'.date("His_Ymd").".xlsx";	
