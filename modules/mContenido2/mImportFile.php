...
function mensaje_error(){
	...
	//Mensajes de error unidades
		if(count($sap_no)>0){
				$mensaje .=  '<table border="0" width="100%">';
				$mensaje .= '<tr><td align="center">Cliente(s) Inexistente(s)</td></tr>';	 
				$mensaje .= '<tr><td>';
...
function generar_data_dsp(){
	...
	$excel = new Spreadsheet_Excel_Reader($archi);
		...
		for ($row=2;$row<=$excel->rowcount();$row++) {
			//echo $excel->val($row,'B')."<br>\n";
			if($row == 2){
				$itm = "".$excel->val($row,'B')."";
				$fiv = $excel->val($row,'E');
				}
			else{
				//echo "".$excel->val($row-1,'B')." != ".$excel->val($row,'B')."";
				if("".$excel->val($row-1,'B')."" != "".$excel->val($row,'B').""){
					$ffv = $excel->val($row-1,'F');
					//Generar cadena
