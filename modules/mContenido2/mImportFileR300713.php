...
function validar_data(){
	...
	for ($row=2;$row<=$excel->rowcount();$row++) {
		//Validar Cliente
	//if($excel->val($row,'D')!=""){
		if ($excel->val($row,'C')!="" && !in_array($excel->val($row,'C'), $sap_it)){
		$sap_no[]=$excel-> val($row,'C');
		}
		//else{
			if($excel->val($row,'C')!="" && !in_array($excel->val($row,'C'), $sap_in)){
		$sap_in	[] = $excel-> val($row,'C');
			}
			//}
	//}	
		//Validar unidades
	//if($excel->val($row,'G')!=""){
		if ($excel->val($row,'D')!="" && !in_array($excel->val($row,'D'), $und_nm) && !in_array($excel->val($row,'D'), $und_no)){
		$und_no[]=$excel-> val($row,'D');
		}
		else{
			//if(!in_array($excel->val($row,'G'), $und_ok)){
		$und_ok [] = $excel-> val($row,'D');
			//}
			}
	//}	
		//Validar entregas
	...
	//}
		...
		//Validar fecha estimada arribo
			if ($excel->val($row,'E')!="" && !preg_match($erf, $excel->val($row,'E'))){
			$hr_no[]=$excel-> val($row,'E');
			}		
		...
		//Validar fecha estimada de salida
			if ($excel->val($row,'F')!="" && !preg_match($erf, $excel->val($row,'F'))){
			$hr_no[]=$excel-> val($row,'F');
			}
		...
		//Comparación de fechas
			...
			$date_a = new DateTime($excel->val($row,'E'));
			$date_a = date_format($date_a, 'Y-m-d H:i:s');	
			$date_b = new DateTime($excel->val($row,'F'));
			...
			//FECHA ESTIMADA FINALIZACION NO DEBE SER MAYOR A FECHA ESTIMADA ARRIBO
			if($date_b < $date_a){
			$dates++;
			$emi[]=$date_b;
				}	
...
///////////////////////////////////////////////////////////////////
function get_id($id,$tbl,$whr){
	global $db;
	$sql = "SELECT ".$id." AS ID FROM ".$tbl.$whr;
	$qry = $db->sqlQuery($sql);
	$row = $db->sqlFetchArray($qry);
	return $row['ID'];
	}
//-----------------------------------------------------------------
function generar_data_iti(){
	global $archi,$idu;
	$data = "";
	require_once 'excel_reader2.php';
	$excel = new Spreadsheet_Excel_Reader($archi);
	...
	for ($row=2;$row<=$excel->rowcount();$row++) {
		...
		$data .= ($data=="")
		?" (".$id.",4,".$go.",".$idu.",'".$excel->val($row,'C')."','".$excel->val($row,'E')."','".$excel->val($row,'F')."','".date('Y-m-d H:i:s')."')"
		:",(".$id.",4,".$go.",".$idu.",'".$excel->val($row,'C')."','".$excel->val($row,'E')."','".$excel->val($row,'F')."','".date('Y-m-d H:i:s')."')";
		}
	return $data;	
	}
//----------------------------------------------------------------
function generar_data_und(){
	...
	$excel = new Spreadsheet_Excel_Reader($archi);
		...
		for ($row=2;$row<=$excel->rowcount();$row++) {
			if($row== 2 && $excel->val($row,'B')!=""){
				...
				if($excel->val($row,'B')!="" && $excel->val($row,'B') != $excel->val($row+1,'B')){
					
					//Generar cadena
					//ID_DESPACHO, COD_ENTITY, FECHA_ASIGNACION, ACTIVO, LIBRE
					$data .= ($data == "")
					?" (".$id.",".$un.",'".date('Y-m-d H:i:s')."',1,0)"
					:",(".$id.",".$un.",'".date('Y-m-d H:i:s')."',1,0)";
					//reasignar variable
					//$idv_tmp = $excel->val($row,'B');
					$id = ($excel->val($row+1,'B')!="")
					?get_id("ID_DESPACHO","DSP_DESPACHO"," WHERE ITEM_NUMBER = '".$excel->val($row+1,'B')."'"):"";
					$un = ($excel->val($row+1,'B')!="")
					?get_id("COD_ENTITY","SAVL1120"," WHERE DESCRIPTION = '".$excel->val($row+1,'D')."' AND COD_CLIENT = ".$idc):"";
					}
				}	
			}
	return ($data);	
		...
		for ($row=2;$row<=$excel->rowcount();$row++) {
			if($row == 2){
				...
				if($excel->val($row,'B') != $excel->val($row+1,'B')){
					$ffv = $excel->val($row,'F');
					//Generar cadena
					//`ID_ESTATUS``COD_USER``DESCRIPCION``ITEM_NUMBER``FECHA_INICIO``FECHA_FIN``CREADO`
					$data .= ($data == "")
					?" (2,".$idu.",'".$itm."','".$itm."','".$fiv."','".$ffv."','".date('Y-m-d H:i:s')."')"
					:",(2,".$idu.",'".$itm."','".$itm."','".$fiv."','".$ffv."','".date('Y-m-d H:i:s')."')";
					//reasignar variables
					$itm = $excel->val($row+1,'B');
					$fiv = $excel->val($row+1,'E');
					}
				}	
			}
	return ($data);
...
function almacenar_data(){
	global $db, $mensaje;
	$cad_dsp = generar_data_dsp();
	if($cad_dsp !=""){
		$sql_d = "INSERT INTO DSP_DESPACHO
		(ID_ESTATUS,COD_USER,DESCRIPCION,ITEM_NUMBER,FECHA_INICIO,FECHA_FIN,CREADO)VALUES".$cad_dsp;
		$qry_d = $db->sqlQuery($sql_d);
			if($qry_d==true){
				$r = explode("(",$cad_dsp);
				$mensaje .= '<table border="0" width="100%">';
				$mensaje .= '<tr><td>';
				$mensaje .="Se insertaron ".(count($r)-1)." viaje(s) con exito";
				$mensaje .='</td></tr>';
				$mensaje .='</table>';
				}
		}
	$cad_iti = generar_data_iti();
	if($cad_iti != ""){
		$sql_e = "INSERT INTO DSP_ITINERARIO
		(ID_DESPACHO,ID_ESTATUS,COD_GEO,COD_USER,ITEM_NUMBER,FECHA_ENTREGA,FECHA_SALIDA,CREADO) VALUES ".$cad_iti;
		$qry_e = $db->sqlQuery($sql_e);
			if($qry_e==true){
				$r = explode("(",$cad_iti);
				$mensaje .= '<table border="0" width="100%">';
				$mensaje .= '<tr><td>';
				$mensaje .="Se insertaron ".(count($r)-1)." visita(s) con exito";
				$mensaje .='</td></tr>';
				$mensaje .='</table>';
				}		
		}
	$cad_und = generar_data_und();
	if($cad_und !=""){
		$sql_f = "INSERT INTO DSP_UNIDAD_ASIGNADA
		(ID_DESPACHO, COD_ENTITY, FECHA_ASIGNACION, ACTIVO, LIBRE) VALUES ".$cad_und;
		$qry_f = $db->sqlQuery($sql_f);
			if($qry_f==true){
				$r = explode("(",$cad_und);
				$mensaje .= '<table border="0" width="100%">';
				$mensaje .= '<tr><td>';
				$mensaje .="Se asiganaron ".(count($r)-1)." unidad(es) al viaje con exito";
				$mensaje .='</td></tr>';
				$mensaje .='</table>';
				}		
		}
