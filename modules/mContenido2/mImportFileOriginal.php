...
$er="/(0[1-9]|1[0-9]|2[0-3]):([0-5][0-9]:[0-5][0-9]|(59|44|29):60) (p.m.|a.m.)?/i";	
$ert="/(0[1-9]|1[0-9]|2[0-3]):([0-5][0-9]):?([0-5][0-9])? ?(p.m.|a.m.)?/i";
$ert="/(0[1-9]|1[0-9]|2[0-3]):([0-5][0-9]):?([0-5][0-9])? ?/i";
$erf="/(0[1-9]|1[0-9]|2[0-9]|3[0-1]).(0[1-9]|1[0-2]).?(\d{4})?/i";
for ($row=2;$row<=$excel->rowcount();$row++) {
	
	/*if($date_c < $date_a){
		if (!preg_match($er, $excel->val($row,'A'))){
		$hr_no[]=$excel-> val($row,'A');
		}
		//$em[]=
		$mensaje.=$date_c." < ".$date_a;
	}*/
//////////////////////////////////////////////////////	
	if($excel->val($row,'A')!=""){
		if (!preg_match($er, $excel->val($row,'A'))){
...
//////////////////////////////////////////////////////
	if($excel->val($row,'K')!=""){
		if (!preg_match($er, $excel->val($row,'K'))){
		$hr_no[]=$excel-> val($row,'K');
		}
	}
//////////////////////////////////////////////////////
	if($excel->val($row,'L')!=""){
		if (!preg_match($er, $excel->val($row,'L'))){
		$hr_no[]=$excel-> val($row,'L');
		}
	}
///////////////////////////////////////////////////////
	if($excel->val($row,'N')!=""){
		if (!preg_match($er, $excel->val($row,'N'))){
		$hr_no[]=$excel-> val($row,'N');
		}
	}		
}
...
$date_a = new DateTime($excel->val($row,'J')." ".$excel->val($row,'A'));
$date_a = date_format($date_a, 'Y-m-d H:i:s');	
$date_b = new DateTime($excel->val($row,'J')." ".$excel->val($row,'N'));
...
$date_d = new DateTime($excel->val($row,'J')." ".$excel->val($row,'L'));
	...
	//Validar despachos
		...
		for ($row=2;$row<=$excel->rowcount();$row++) {
			...
			if($excel->val($row,'B')!=""){
				...
				$vls=($vls=="")?"(4,".$idu.",'".$excel->val($row,'B')."','".$excel->val($row,'B')."',0,'".$dti."','".$dtf."','".date('Y-m-d H:i:s')."')":$vls.",(4,".$idu.",'".$excel->val($row,'B')."','".$excel->val($row,'B')."',0,'".$dti."','".$dtf."','".date('Y-m-d H:i:s')."')";
				$r++;
				}
				else{
...
$ce=array();
		...
		$sql_e = "INSERT INTO DSP_ITINERARIO (ID_DESPACHO,ID_ESTATUS,COD_GEO,COD_USER,ITEM_NUMBER,COMENTARIOS,FECHA_ENTREGA,FECHA_FIN,FECHA_SALIDA,VOLUMEN,ID_TIPO_VOLUMEN,CREADO) VALUES".$vls_i;
