...
/*-------------------------------------------------------------------------*/	
	public function obtener_ureporte_2($id_unit,$fecha){
		global $config_bd2;
...
public function obtener_ureporte_222($id_unit,$fecha){
		global $config_bd2;
		$table_last = $this->get_table_hist($id_unit);
		$conta=0;
		$conexion = mysqli_connect($config_bd2['host'],$config_bd2['user'],$config_bd2['pass'],$config_bd2['bname']);
		if($conexion){
	 $sql	= "SELECT f.PLAQUE,f.DESCRIPTION,
			                  IF (e.GPS_DATETIME IS NULL, '0000-00-00 00:00:00',e.GPS_DATETIME) AS GPS_DATETIME ,
							  IF(g.DESCRIPTION IS NULL,'NO HA REPORTADO',g.DESCRIPTION) AS DESC_EVT,
							  IF(e.VELOCITY IS NULL,0,e.VELOCITY) AS VELOCITY,
							  f.COD_FLEET,
...
for($a=0;$a<count($cant_histo);$a++){			
 $sql = "SELECT e.COD_ENTITY,CONCAT(g.DESCRIPTION,'-',g.PLAQUE) AS UNIT, CAST(e.GPS_DATETIME AS DATE ) AS FECHA , f.DESCRIPTION AS EVT, e.LATITUDE, e.LONGITUDE, e.VELOCITY, e.ANGLE,e.GPS_DATETIME
	...
	function get_num_hist3_x($hist, $idunit, $rtime, $filtro){
		...
		if($conexion){
for($a=0;$a<count($cant_histo);$a++){			
 $sql = "SELECT e.BATTERY,e.COD_ENTITY,CONCAT(g.DESCRIPTION,'-',g.PLAQUE) AS UNIT, CAST(e.GPS_DATETIME AS DATE ) AS FECHA , f.DESCRIPTION AS EVT, e.LATITUDE, e.LONGITUDE, e.VELOCITY, e.ANGLE,e.GPS_DATETIME
						 FROM ".$cant_histo[$a]." e
						 LEFT JOIN SAVL1260 f ON f.COD_EVENT = e.COD_EVENT
						 LEFT JOIN SAVL1120 g ON g.COD_ENTITY = e.COD_ENTITY
						 WHERE e.COD_ENTITY IN (".$idunit.")
...
IF(e.LONGITUDE IS NULL,0,e.LONGITUDE) AS LONGITUDE,
	...
	public function obtener_ult_report($id_unit){
		global $config_bd2;
		$table_last = $this->get_table_upos($id_unit);
		$conexion = mysqli_connect($config_bd2['host'],$config_bd2['user'],$config_bd2['pass'],$config_bd2['bname']);
		if($conexion){
			$sql	= "SELECT f.PLAQUE,f.YEAR,f.DESCRIPTION,e.GPS_DATETIME,g.DESCRIPTION AS DESC_EVT,e.VELOCITY,f.COD_FLEET,
			                   e.LATITUDE,e.LONGITUDE,e.COD_ENTITY,g.PRIORITY, g.FLAG_CRM, e.ID_RES_CUESTIONARIO
						FROM  ".$table_last." e
...
/*----------------------------------------------------------------------------*/	
	...
	public function obtener_reporte_det($id_unit){
		...
		if($conexion){
			$sql	= "SELECT f.PLAQUE,f.YEAR,f.DESCRIPTION,e.GPS_DATETIME,g.DESCRIPTION AS DESC_EVT,e.VELOCITY,f.COD_FLEET,
			                   e.LATITUDE,e.LONGITUDE,f.COD_ENTITY
						FROM  ".$table_last." e
						LEFT JOIN SAVL1120 f ON e.COD_ENTITY = f.COD_ENTITY
						LEFT JOIN SAVL1260 g ON e.COD_EVENT  = g.`COD_EVENT`
						WHERE e.COD_ENTITY = ".$id_unit."
...
function get_num_hist24($idunit, $rtime, $filtro){
  ...
  $sql="SELECT  D.ID_DESPACHO,D.DESCRIPCION AS RUTA, U.DESCRIPTION AS UND, E.DESCRIPCION AS EST, D.FECHA_INICIO, D.FECHA_FIN, (SELECT COUNT(I.ID_ENTREGA)  FROM DSP_ITINERARIO I WHERE I.ID_DESPACHO = D.ID_DESPACHO) AS TPV, D.FECHA_REAL_INICIO, D.FECHA_REAL_FIN,D.DIF_INICIO, D.DIF_FIN,D.TOTAL_DISTANCIA,D.TOTAL_EXCESOS,D.TOTAL_PARADAS FROM DSP_DESPACHO D INNER JOIN DSP_UNIDAD_ASIGNADA UA ON UA.ID_DESPACHO = D.ID_DESPACHO INNER JOIN SAVL1120 U ON U.COD_ENTITY = UA.COD_ENTITY INNER JOIN DSP_ESTATUS E ON E.ID_ESTATUS = D.ID_ESTATUS WHERE U.COD_ENTITY=".$idunit.$rtime.$filtro;
