...
for ($row=3;$row<=$excel->rowcount();$row++) {
	if($excel->val($row,'B')!=""){
...
if(count($sap_no)==0 && count($und_no)==0 && count($ent_ex)==0 && $du==0){
//OBTENGO IDS
//ID SAP CLIENTE
	...
	//Validar despachos
		...
		if($vls!=""){
			...
			if($qry_d==true){
				...
				//$mensaje .= '<tr><td align="center">Pedido(s) Inexistente(s)</td></tr>';	 
				$mensaje .= '<tr><td>';
				$mensaje .="Se insertaron ".count($dsp2)." viaje(s) con exito";
				$mensaje .='</td></tr>';
				$mensaje .='</table>';
...
$ce=array();
		for ($row=2;$row<=$excel->rowcount();$row++) {
			if($excel->val($row,'B')!=""){
				...
				$vls_i = ($vls_i=="")?"(".$row_h['ID_DESPACHO'].",4,".$row_i['COD_OBJECT_MAP'].",".$idu.",".$excel->val($row,'E').",".$ob.",".$dte.",'".date('Y-m-d H:i:s')."')":$vls_i.",(".$row_h['ID_DESPACHO'].",4,".$row_i['COD_OBJECT_MAP'].",".$idu.",".$excel->val($row,'E').",".$ob.",".$dte.",'".date('Y-m-d H:i:s')."')";
				$d++;
				}
		}
		/*for ($x=0;$x<=count($und_ok);$x++) {
