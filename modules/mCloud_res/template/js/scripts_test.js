	...
	function(action) {
		...
		if(key_prin1 != 0){
			...
			//alert(bora);
			    var ms_texto = "";
				if(borrar_menu != 0){
					ms_texto =' Desea eliminar el elemento de menu y todo su contenido?'
				}
			    if(borrar_submenu != 0){
					ms_texto =' Desea eliminar el submenu y todo su contenido?'
				}
			document.getElementById('dialog1').innerHTML="<p align='center'>"+ms_texto+"</p>";		
...
for(i=0;i<arraydocs.length;i++){
	...
	function(action) {
		...
		if(key_prin != 0){
			...
			switch(action){
				...
				//alert(borrar_parametros);
				var a_texto = "";
				if(borrar_parametros == 'url'){
					a_texto = 'Desea eliminar el enlace url?'
				}else{
					a_texto = 'Desea eliminar el archivo?'
				}
				document.getElementById('dialog1').innerHTML="<p align='center'>"+a_texto+"</p>";		
	...
	//------------------------- la funcion recibe respuesta de 1, si se ha subido y copiado por ftp el archivo..
	  function hola(x,dr,ddet,id_m){
	//	alert(dr);
		if(x=='1'){
...
'<tr style="height:120px; display:'+visible+';" ><td bgcolor="#c5dbec" >Resumen</td> <td><textarea  style="width:100%; height:100%;"   class="caja_txt"  id="resumen" name="resumen"> </textarea></td></tr>'+
...
'<tr style="display:'+visible+';"><td bgcolor="#c5dbec" width="120">Evento</td><td>'+
      ...
      '<div id ="selex"> <select style="width:100%;"  class="caja_txt" ><option>Cargando Datos... </option></select></div>'+
		...
		alert('Debe de elegir un tipo de Archivo o especificar URL');
	}	
}
...
function elimina_fil(x,t,cat,borrar_parametros){
		...
		var ulr=convert(x);
		$("#letra_x").html("Eliminando datos espere un momento");
		$("#dialog_prog").dialog("open");
	...
	//var url = "index.php?m=rRsalida&c=mGetReport";
		...
		//alert(result);
				...
				$("#dialog_prog").dialog("close");	
					if(result!=2){	
						document.getElementById('dialog2').innerHTML='<p align="center">El archivo ha sido eliminado</p>';	
					}else{
						document.getElementById('dialog2').innerHTML='<p align="center">El elemento fue eliminado</p>';
					}
...
function editphp(urls){
	...
	if(v1 == '1'){
		$nuevo_elemento = "Nuevo Elemento de Menú";
	}
	if(v2 == '1'){
		$nuevo_elemento = "Nuevo Submenu";
	}
...
//--------------------------------------- funciones de catalogo
	...
	//alert($('#des_catalogo').val()+' '+ clientes);
	var ajax = nuevoAjax();
	...
	ajax.open("GET", "index.php?m=mCloud&c=mUsuarioSubmenu&id_submenu="+id_submenu,true);
		...
		if (ajax.readyState==4) {
				var result =ajax.responseText;
				  ...
				  if(result!=0){
					 $("#conte_1").html(result);  
					 	 $("#bandera_guarda_asignado").val(1); 
				  }else{
					 $("#conte_1").html(htlm2);  
					 	 $("#bandera_guarda_asignado").val(0); 
				  }
...
function elegir_tipo_archivo(){
	    $('input[name=tiposs]').attr('checked',false);
	 ...
	 var msn_url = '&nbsp;&nbsp;  Nota: <br>'+ 
		...
		// $("#conte_2").html(disp);
		 //$("#mensaje_url").html(msn_url);
	   	 $("#bandera_guardar_detalles").val(0);
...
var def = '<table border="0" width="100%" height="100%" > '+ 
     ...
     '   </tr>'+ 
        ...
        '  <td></td>'+ 
        '  <td><input type="button" value="Guardar Cambios"  onClick="guardar_cambios();" class="boton ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only"/></td>'+ 
        '  <td >&nbsp;</td>'+ 
        '</tr> '+   
'</table>';	
...
//------------------------ funcion que realiza la inserción de datos de contenido, y detalle

function GuardaDatos(cont,det,id_men){
	
		
    var ajax = nuevoAjax();
        ajax.open("GET", "index.php?m=mCloud&c=mGuardaContDet&contenido="+cont+"&detalle="+det+"&id_menus="+id_men,true);
   	    ajax.onreadystatechange=function() {
		    if (ajax.readyState==4) {
				var result =ajax.responseText;
				  console.log(result); 
