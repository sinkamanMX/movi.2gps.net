...
//$nuevo_testo = rename("catalogos/archivo.url", "/home/user/login/docs/mi_archivo.txt");
 ...
 // $moved2 = move_uploaded_file($_FILES['foto']['tmp_name'],'/catalogos/imagenes_detalles/'.reemplaza(str_replace(" ","_",utf8_decode($_FILES['foto']['name']))));
//							if($moved2) {		   		
//								if(guardar($_FILES['archivo']['name'],$utl,$_POST['tip_archi']),$_FILES['foto']['name']){
//							       $T=1;
...
function guardar($nom_ar,$utl,$tpa,$img_det){
	...
	$sqlZ = "INSERT INTO CAT_CONTENIDO(ID_SUBMENU,UBICACION_LOCAL,UBICACION_REMOTA,DESCRIPCION,ORDEN)
		   ...
		   if($queri){
  	     	...
  	     	if(guardar_detalle($row_ultimo['IDCONT'],$_POST['titulo'],$_POST['autor'],$_POST['resumen'],$_POST['tag'],$_POST['eventos']),$img_det){
		   	     		  $T = 1;
		   	     	}else{
		   	     		  $T = 0;
		   	     	}
...
//------------------------------ guarda en cat_contenido-detalle, y actualiza en cat_submenu si es url
 ...
 function guardar_detalle($id,$titulo,$autor,$resumen,$tag,$ev,$img_det){
  	global $db;
  	$detalle = "INSERT INTO CAT_CONTENIDO_DETALLE 
				(ID_CONTENIDO,TITULO, 
                 ...
                 VALUES('".$id."','".$titulo."','".$autor."','".$resumen."','".$tag."','".$ev."','".$img_det."')";
