...
class mFunciones_CV{
	...
	function convierte_horas($seg){
    	...
    	$seconds   = $seg % 60;
		$min_trans = sprintf("%d:%02d:%02d", $hours, $minutes, $seconds); 
		return $min_trans;	 
	}
  ...
  function datos_unidad($unidad){
  	global $db;
  	$sql = "SELECT CONCAT(DESCRIPTION,'-',PLAQUE) AS UNIDAD
			FROM SAVL1120 			
