	...
	function clear_div(){
		if(units_s.length!=0 || list_u2.length!=0 ||email2.length!=0 ){
				...
				for(i=0;i<list_s2.length;i++){
						...
						var pos =list_u2.indexOf( list_s2[i]);
								if(pos==-1){
									list_u2.push(list_s2[i]);
									}
			...
			busk_nue3='';
					...
					//var url = "index.php?m=rRsalida&c=mGetReport";
						...
						if (ajax.readyState==4) {
								...
								'<tr><td colspan="2"><select id="mod_va" style="width:280px;" ></select></td></tr></table></div>';	
					...
					//var url = "index.php?m=rRsalida&c=mGetReport";
						...
						if (ajax.readyState==4) {
							...
							Ok: function() {
								...
								noms_edi.splice( pos, 1 );
								lista_edi.splice( pos, 1 );
				  				$( this ).dialog( "close" );
