...
function r_filtro1(){
	...
	//var url = "index.php?m=rRsalida&c=mGetReport";
		ajax.open("GET", "index.php?m=mAlertas&c=mGetPoints",true);
...
function r_filtro3(){
	...
	var ajax = nuevoAjax();
		...
		ajax.open("GET", "index.php?m=mAlertas&c=mGetPGeo",true);
		ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
				var result =ajax.responseText;
				//alert(result);
	...
	//list_u=new Array();
		for(i=0;i<grup.length;i++){
			...
			if(parametro[0]==x){
				...
				divs+='<tr id="'+parametro[2]+'"><td>'+parametro[3]+'</td><td><a href="#"  onclick="listar_objeto(\''+parametro[2]+'\',\''+parametro[3]+'\')" ><img title="click" src="/public/images/d_arrow.png" style=" height:12px; width:12px;"></a></td></tr>';
				}
...
function listar_objeto(s,y){
			...
			var divs='<tr id="'+s+'"><td><a href="#"  onclick="quitar_objeto(\''+s+'\',\''+y+'\')" ><img title="click" src="/public/images/i_arrow.png" style=" height:12px; width:12px;"></a></td><td>'+y+'</td></tr>';
			list_s2.push(divs);
...
function quitar_objeto(s,y){
			...
			var divs='<tr id="'+s+'"><td>'+y+'</td><td><a href="#"  onclick="listar_objeto(\''+s+'\',\''+y+'\')" ><img title="click" src="/public/images/d_arrow.png" style=" height:12px; width:12px;"></a></td></tr>';
			list_u2.push(divs);
...
function r_filtro2(){
	var ajax = nuevoAjax();
		...
		ajax.open("GET", "index.php?m=mAlertas&c=mGetPunits",true);
		ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
				var result =ajax.responseText;
					//alert(result);
				...
				var divs="";
					...
					for(x=0;x<parametro.length;x++){
							...
							list_u.push(sel[2]);
							list_un.push(sel[3]);
							var divs2='<tr id="'+sel[2]+'"><td>'+sel[3]+'</td><td><a href="#"  onclick="listar_objeto(\''+sel[2]+'\',\''+sel[3]+'\')" ><img title="click" src="/public/images/d_arrow.png" style=" height:12px; width:12px;"></a></td></tr>';
							list_u2.push(divs2);
							divs+='<tr id="'+sel[2]+'"><td>'+sel[3]+'</td><td><a href="#"  onclick="listar_objeto(\''+sel[2]+'\',\''+sel[3]+'\')" ><img title="click" src="/public/images/d_arrow.png" style=" height:12px; width:12px;"></a></td></tr>';
				...
				for(i=0;i<list_s.length;i++){
						...
						var pos =list_u.indexOf( list_s[i]);
								if(pos==-1){
									...
									var divs='<tr id="'+list_s[i]+'"><td>'+list_sn[i]+'</td><td><a href="#"  onclick="listar_objeto(\''+list_s[i]+'\',\''+list_sn[i]+'\')" ><img title="click" src="/public/images/d_arrow.png" style=" height:12px; width:12px;"></a></td></tr>';
									list_u2.push(divs);
									}
					}
	...
	function pestana1(){
		...
		//alert(iconos[0]);
				...
				if(contador<12){
						...
						ides.push(sl);
							...
							'<div id="etic" ></div>'+
             				...
             				'<img id="ico" src="'+lista1[8]+'" alt="The peaks of High Tatras" style="height:13%;" />'+
			...
			for(i=0;i<contador2;i++){
				if(boton==' '){
					boton='<td><a href="#" name="'+i+'" onclick="chekar(this.name)" ><img title="" src="/public/images/ss.png" style=" width:15px;"></a><td>';
				}else{
					
					boton=boton+'<td>&nbsp;</td>'+'<td><a href="#" name="'+i+'" onclick="chekar(this.name)" ><img title="" src="/public/images/ss.png" style=" width:15px;"></a><td>';
					}
		...
		if(sdta==0){
			...
			document.getElementById('mod_va').innerHTML= busk_nue3;
				...
				for(i=0;i<units_s.length;i++){
					...
					if(units_s[i]==x){
						for(x=0;x<busk_nue1.length;x++){
									var seletsion='';
									if(nuve[i]==busk_nue1[x]){
											seletsion='<option value="'+busk_nue1[x]+'" selected="selected">'+busk_nue2[x]+'</option>';
						
						
										}else{
											seletsion='<option value="'+busk_nue1[x]+'">'+busk_nue2[x]+'</option>';
...
function popup_mod(id,desp,icon,val,def,type,opc){
		...
		if(type=='BD'){
			...
			busk_nue3='';
					...
					//var url = "index.php?m=rRsalida&c=mGetReport";
						ajax.open("GET", "index.php?m=mAlertas&c=mNewQuery&q="+opc,true);
						ajax.onreadystatechange=function() {
						if (ajax.readyState==4) {
								var result =ajax.responseText;
								...
								'<tr><td colspan="2"><input type="text" size="30" id="bup" title="Buscar" onkeyup="txts(this.value)" /><a href="#" onclick="" ><img title="Buscar" src="/public/images/search.png" style=" width:18px;"></a></td></tr>'+
...
var dialogo_conf='';
	if(listo==0){
		var dialogo_conf='<div id="accordion"><table border="0" ><tr><td colspan="2" height="20" width="450">'+
		 ...
		 '</tr><tr ><td colspan="2" height="40" >'+
  		'<div style=" height:224px;">'+
			...
			'<td rowspan="3" style="heigth:330px; "><div>'+
		     '<div id="sizer1" class="sizer" style="width:340px; heigth:310px; position:relative; top:20;" ><p>'+
			...
			'<div id="sizer" class="sizer" style="width:400px; position:relative; top:20;" ><p>'+
	...
	/*class="sizer"*/
		...
		dialogo_conf='<div id="accordion"><table border="0" ><tr><td colspan="2" height="20" width="450">'+
    	 '<font color="#000000" face="Arial Black" size="2" >Datos </font></td><td>&nbsp;</td>'+ 
		 '<td height="20"><font color="#000000" face="Arial Black" size="2" >Unidades </font></td>'+
		 '</tr><tr ><td colspan="2" height="40" >'+
  		'<div style=" height:224px;">'+
			...
			'<td rowspan="3" style="heigth:310px; "><div>'+
		     '<div id="sizer1" class="sizer" style="width:330px; heigth:310px; position:relative; top:20;" ><p>'+
			 '<table><tr><td colspan="3"><div style=" position:relative; width:305px; heigth:10px;" id="nuw" class="styled-select">'+unidades_selectud+'</div></td></tr>'+
			  '<tr><td><input type="button" style="" id="bu_guar" class="ui-button" value="Asignar Todos"  onclick="enviar_todos()"/></td><td>&nbsp;&nbsp;</td><td><input type="button" style="" id="bu_guar" class="ui-button" value="Quitar Todos"  onclick="regresar_todos()"/></td></tr>'+
			 '<tr><td>Unidades Disponibles</td><td>&nbsp;&nbsp;</td><td>Unidades Asignadas</td></tr>'+
 			'<tr><td ><div id="col_uni" style="border-radius: 7px; background-color:white; overflow-x:hidden; overflow:auto; width:155px; color:black;  height:300px; position:absolute;" ><table id="unidad_l"  > </table></div></td>'+
			...
			for(i=0;i<list_u.length;i++){
				var pos = list_s.indexOf(list_u[i]);
					if(pos==-1){
						...
						var divs='<tr id="'+list_u[i]+'"><td><a href="#"  onclick="quitar_objeto(\''+list_u[i]+'\',\''+list_un[i]+'\')" ><img title="click" src="/public/images/i_arrow.png" style=" height:12px; width:12px;"></a></td><td>'+list_un[i]+'</td></tr>';
						list_s2.push(divs);
						$( divs ).appendTo("#col_sel_uni");
...
function nuew_cor(){
			...
			if(vali){
					if(exist_c(correo)){
						conten=conten+1;
						email.push(correo);
						dato='<tr id="tbd'+conten+'"><td>'+correo+'</td><td><a href="#" onclick="delet_corre(\'tbd'+conten+'\',\''+correo+'\')" ><img title="click" src="/public/images/elimina.png"></a></td></tr>';
						email2.push(dato);
						$(dato).appendTo("#tab_og");
						}else{
...
function val_nombre(){
	...
	var nombre=$("#id_nom").val();
						var ajax = nuevoAjax();
					//var url = "index.php?m=rRsalida&c=mGetReport";
						ajax.open("GET", "index.php?m=mAlertas&c=mNewQuery2&q="+nombre,true);
						ajax.onreadystatechange=function() {
						if (ajax.readyState==4) {
								var result =ajax.responseText;
			 ...
			 if(listo1!=0){
				 var arreglo=respuesta.toString();
						...
						var url="index.php?m=mAlertas&c=mGetGuard&q="+arreglo;
							...
							var url="index.php?m=mAlertas&c=mGetGuard2&q="+arreglo+"&d="+listo2;
							//alert(url);
							}
						...
						if (ajax.readyState==4) {
								...
								document.getElementById('dialogo_res').innerHTML=alertk;
...
function r_filtro5(){
	document.getElementById('cvs').innerHTML='<tr><td align="center" ><font color="#000" face="Arial Black" size="2"  >Cargando...</font></td></tr>';
		var ajax = nuevoAjax();
					//var url = "index.php?m=rRsalida&c=mGetReport";
						ajax.open("GET", "index.php?m=mAlertas&c=mGetraDat",true);
						ajax.onreadystatechange=function() {
						if (ajax.readyState==4) {
								var result =ajax.responseText;
								if(result != 0){
								...
								document.getElementById('cvs').innerHTML='<tr><td><div id="sizer2" class="sizer" style="width:230px;" >'+
			...
			$( "#dialog" ).dialog({
			  ...
			  buttons: {
				Aceptar: function() {
					var ajax = nuevoAjax();
					//var url = "index.php?m=rRsalida&c=mGetReport";
						ajax.open("GET", "index.php?m=mAlertas&c=mDelDat&q="+x,true);
						ajax.onreadystatechange=function() {
						if (ajax.readyState==4) {
								var result =ajax.responseText;
...
function editar_todo(dato){
	...
	for(d=0;d<corr_ne.length;d++){
		...
		var dt='<tr id="tbd'+d+'"><td>'+corr_ne[d]+'</td><td><a href="#" onclick="delet_corre(\'tbd'+d+'\',\''+corr_ne[d]+'\')" ><img title="click" src="/public/images/elimina.png"></a></td></tr>';
	email2.push(dt);
	}
	...
	var divs='<tr id="'+id_unids[f]+'"><td><a href="#"  onclick="quitar_objeto(\''+id_unids[f]+'\',\''+nom_unid[f]+'\')" ><img title="click" src="/public/images/i_arrow.png" style=" height:12px; width:12px;"></a></td><td>'+nom_unid[f]+'</td></tr>';
			list_s2.push(divs);
