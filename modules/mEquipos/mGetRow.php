...
**/
	...
	$where = 'COD_ENTITY NOT IN (
			    ...
			    WHERE COD_CLIENT = '.$userAdmin->user_info['ID_CLIENTE'].') AND COD_CLIENT = '.
				$userAdmin->user_info['ID_CLIENTE'];			    
	...
	$query_evts_eq = ' FROM ADM_EVENTOS_EQUIPOS A
		...
		WHERE B.COD_EVENT_EQUIPMENT IS NULL ORDER BY A.EVENT_REASON ';
	...
	$query_sistema = ' FROM ADM_EVENTOS A
		...
		WHERE B.COD_EVENT IS NULL AND
      	A.ID_CLIENTE = '.$userAdmin->user_info['ID_CLIENTE'].' ORDER BY A.DESCRIPTION';
	...
	$grupo = $dbf->cbo_from_query('G.ID_GRUPO',' G.NOMBRE',' FROM ADM_GRUPOS G
									...
									WHERE GC.ID_CLIENTE= '.$userAdmin->user_info['ID_CLIENTE'].' ORDER BY G.NOMBRE');	
