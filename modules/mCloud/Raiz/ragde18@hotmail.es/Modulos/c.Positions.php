			...
			while($row = @mysqli_fetch_array($query)){					 
				 ...
				 $arreglo[$conta][1] = $row['COD_EVENT'];
				  $arreglo[$conta][2] = $row['DES_UNID'];
				   $arreglo[$conta][3] = $userAdmin->codif($row['COD_ENTITY']);
			$conta++;
			}
			return $arreglo;
			mysqli_close($conexion);			
...
IF((SELECT MIN(DISTANCIA (P.LATITUDE,P.LONGITUDE,E.LATITUDE,E.LONGITUDE)) FROM SAVL_G_PRIN P WHERE  P.COD_CLIENT = ".$cliente.")<".$radio.",1,0) AS PARADAS
FROM ".$cant_histo[$a]." E
LEFT JOIN SAVL1120 G ON G.COD_ENTITY = E.COD_ENTITY 
WHERE E.COD_ENTITY IN (".$idunit.")
".$rtime."
