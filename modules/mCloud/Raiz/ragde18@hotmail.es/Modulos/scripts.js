...
var lst_evts_u=new Array();
var lst_ev=new Array();
var lst_evts_t=new Array();
var flag_event=0;
function r_filtro2(){
	var ajax = nuevoAjax();
	lst_evts_u=new Array();
	lst_ev=new Array();
	lst_evts_t=new Array();
		...
		if (ajax.readyState==4) {
				...
				if(result != 0){
					...
					var signo=result.split('?');
					//alert(result);
				var parametro=signo[0].split('!');
				var parametro1=signo[1].split('!');
				
				for(i=0;i<parametro1.length;i++){
					var romp=parametro1[i].split('|');
					 lst_evts_t.push([romp[0],romp[1],romp[3]]);
					 
						var pos_evt= lst_ev.indexOf(romp[1]);
						
						if(pos_evt==-1){
							lst_ev.push(romp[1]);
							 lst_evts_u.push([romp[0],romp[1],romp[3]]);
			...
			for(i=0;i<iconos.length;i++){
				...
				if(contador<12){
						...
						ides.push(sl);
							...
							lista.push('<div class="dragbox" name="tip" id="'+sl+'" >'+
							'<h5 class="'+colors+'">'+lista1[2]+'</h5>'+
							'<div id="etic" ></div>'+
			 				'<div class="dragbox-content" >'+
             				'<img id="ico" src="'+lista1[8]+'" alt="The peaks of High Tatras" width="76" height="72" />'+
...
function popup_mod(id,desp,icon,val,def,type,opc){
		...
		if(type=='BD'){
			...
			busk_nue3='';
					...
					//var url = "index.php?m=rRsalida&c=mGetReport";
						...
						if (ajax.readyState==4) {
								...
								'<tr><td colspan="2"><select id="mod_va" style="width:280px;" ></select></td></tr>'+indo+
								'</table></div>';	
			...
			$(".ui-dialog-titlebar-close span", widget).removeClass("ui-icon-closethick").addClass("ui-icon-minusthick");
							...
							if(type=='EVT'){
								var cadena=' ';
								for(i=0;i<lst_evts_u.length;i++){
										
										if(sel_evt_u==lst_evts_u[i][1]){
											cadena+='<option value="'+lst_evts_u[i][1]+'" selected="selected">'+lst_evts_u[i][0]+'</option>';
										}else{
												cadena+='<option value="'+lst_evts_u[i][1]+'">'+lst_evts_u[i][0]+'</option>';
											}
		  ...
		  if(list_u2.length!=0){
			 for(i=0;i<list_u2.length;i++){
					...
					$(dato).appendTo("#unidad_l");
					 ...
					 for(x=0;x<lst_evts_t.length;x++){
					 
					 	if(lst_evts_t[x][1]==sel_evt_u){
							
							var pos_eval=list_u.indexOf(lst_evts_t[x][3]);
							if(pos_eval!=-1){
								var dato=list_u2[post_eval];
								$(dato).appendTo("#unidad_l");
								}
						...
						if (ajax.readyState==4) {
								...
								if(result != 0){
									...
									for(x=0;x<parametro.length;x++){
										...
										var dibuja='<tr id="'+rompe[0]+'"><td ><div id="sizer2" class="sizer" style="width:260px;" ><table  border="0" style="width-min:260px">'+
								...
								document.getElementById('cvs').innerHTML='<tr><td><div id="sizer2" class="sizer" style="width:260px;" >'+
