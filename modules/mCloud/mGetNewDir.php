...
*/  header('Content-Type: text/html; charset=UTF-8');
	...
	if(!mkdir(str_replace(" ","-",$path), 0777, true )){
	  $T=-8;
	}else{
...
function crear_menu($path,$catalogo,$nombre){
	global $db;
 	$sqlZ = "INSERT INTO CAT_MENU(ID_CATALOGO,DESCRIPTION,URL,ACTIVO,ICONO,CREADO,TIPO)
	        VALUES ('".$catalogo."','".str_replace(" ","-",$nombre)."','','S','CARPETA',CURRENT_TIMESTAMP,'M')";
	...
	if($query_mayor){
		...
		if($count>0){
			...
			$num_mayor = $row_mayor['MAXIMO'];
             ...
             VALUES ('".$idmenu."','".str_replace(" ","-",$nombre)."','---','".$num_mayor."','M','F','S','http://movi.2gps.net/catalogos/iconos/icon_s.png')";
		  	$queri = $db->sqlQuery($sqlZ);
