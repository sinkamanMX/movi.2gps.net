...
//$nuevo_testo = rename("catalogos/archivo.url", "/home/user/login/docs/mi_archivo.txt");
   	if (copy('catalogos/enlace.url', $utl.'/'.str_replace("http://"," ",$_POST['urls']).'_'.$_GET['id_menu'])) {
		   ...
		   $T=0;
			...
			if ($_FILES['archivo']["error"] > 0){
			    echo "Error: " . $_FILES['archivo']['error'] . " xxxxx <br>";
				$T=0;
			}else{
			   
	 $destino = "archivos/".reemplaza(str_replace(" ","_",utf8_decode($_FILES['archivo']['name'])));
	 	    
	 $moved   = move_uploaded_file($_FILES['archivo']['tmp_name'],$destino);
				...
				$upload = ftp_put($conexion_id, $archivo_destino, $nombre_archivo, FTP_BINARY);			
				   if ($upload) { 
				   	...
				   	/*
	                    ...
	                    $moved2 = move_uploaded_file($_FILES['foto']['tmp_name'],'catalogos/imagenes_detalles/'. 
 	    		...
 	    		echo "Ha ocurrido un error al subir el archivo mediante ftp checar todo ".$conexion_id.'-'.$archivo_destino.'-'.$nombre_archivo.'<br>';
