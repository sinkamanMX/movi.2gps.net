...
//---------------------------------------   SI  VIENE VACIA LA CAJA DE NOMBRE DE IMAGEN, MANDA ERRORES.	
	if (empty($_FILES['img_login']['name'])){ 
	 ...
	 $moved2 = move_uploaded_file($_FILES['img_login']['tmp_name'],'./catalogos/iconos/'.$_POST['clv_cat_img'].'_'.$_FILES['img_login']['name']);			      if($moved2) {
  	...
  	$res =  guardar_cambios($count,$_POST['clv_cat_img'],$_FILES['img_login']['name'],$_FILES['img_portada']['name'],$_FILES['img_icono']['name']);
					 if($res==1){
					 	$T = 1;
					 }	
		        }else{
...
//------------------------------------------- FUNCION GUARDA CAMBIOS EN LA TABLA CAT_CONTENIDO_DETALLE, LA NUEVA IMAGEN
  function guardar_cambios($cnt,$clv,$logo,$portada,$icono){
  	global $db;
  	  
	if($cnt>0){
		 $ACT = "UPDATE CAT_IMAGENES 
					SET
					IMG_LOGIN = '".$_SERVER['HTTP_HOST']."/catalogos/iconos/".$clv.'_'.$logo."' , 
					IMG_PORTADA = '".$_SERVER['HTTP_HOST']."/catalogos/iconos/".$clv.'_'.$portada."' , 
					IMG_ICONO = '".$_SERVER['HTTP_HOST']."/catalogos/iconos/".$clv.'_'.$icono."'
...
//				VALUES
//				('".$clv."', 
//				 '".$_SERVER['HTTP_HOST']."/catalogos/iconos/".$clv.'_'.$logo."', 
//				 '".$_SERVER['HTTP_HOST']."/catalogos/iconos/".$clv.'_'.$portada."', 
//				 '".$_SERVER['HTTP_HOST']."/catalogos/iconos/".$clv.'_'.$icono."');";
//					
//		     $queria = $db->sqlQuery($ACT); 
//		  	   
//			   if($queria){
